...
namespace Hache.Server.DAO
{
    public class DaoLocales
    {
        ...
        {
            string consulta = ("SELECT ID_Local, Nombre from Locales WHERE Activo = 1");
        ...
        public DataTable ObtenerLocalPorId(int idLocal)
        {
            ...
            string consulta = "SELECT ID_Local, Nombre FROM Locales  WHERE ID_Local = @ID_Local AND Activo = 1";
        ...
        {
            string consulta = "UPDATE Locales SET Activo = 0 WHERE ID_Local = @ID_Local";
            // Crear el parámetro SQL para filtrar por ID
            SqlParameter[] parametros = new SqlParameter[]
            {
                new SqlParameter("@ID_Local", SqlDbType.Int) { Value = idLocal }
